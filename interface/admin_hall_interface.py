...
def admin_hall_interface(all_halls: list[CinemaHall], all_films_names: list[str], hall_id:int) -> tuple[list[CinemaHall], list[str]]:
    while True:
        ...
        show_hall(hall)
        print()
        print(f"""1 - marcar como Ocupada/Deshabilitada/Habilitada una butaca
...
9 - Atras\n""")
        ...
        admin_input = custom_input(
            ...
            validator=lambda option: ("Opcion invalida.", None) if not (option in options_tuple)
                else (None, option)
            )
        ...
        if admin_input == 1:
            ...
            int,
            error_message="Opcion invalida.",
            validator=lambda option: ("Opcion invalida.", None) if not (option in (1, 2, 3))
                else (None, option)
            )
