...
def init_hall():
  ...
  columns:int = custom_input('Ingresa el numero de columnas que tendra la sala: ', int,
    validator=lambda columns: (None, columns) if columns>=1 else ("Minimo 1 columna", None))
  ...
  while option != 2:
    ...
    if seat_status is None:
      print(f"La butaca {row_label}{col_label} ya está inhabilitada.", end="\n"*2)
      continue
  
    if seat_status is False:
      print(f"No se puede inhabilitar: la butaca F{row_label}-C{col_label} está ocupada.", end="\n"*2)
      continue

    disable_seats(hall, row, column)
    print(f"Butaca F{row_label}-C{col_label} inhabilitada correctamente.", end="\n"*2)
