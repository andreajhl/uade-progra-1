...
def input_coords_seat(hall:list[list]):
    ...
    while True:
        ...
        col = input(f"Ingrese la COLUMNA (A-{numbers_into_letters(total_cols - 1)}) para la fila {row}: ").upper()
...
def input_seat_row(hall:list[list]):
    ...
    row = custom_input(f"Ingrese la FILA (1-{total_rows}, -1 para cancelar): ", int,
      validator=lambda row: ("Fila fuera de rango.", None) if(row < 1 and row != -1) or row > total_rows else (None, row-1))
...
def input_seat_column(hall:list[list]):
    ...
    column = custom_input(f"Ingrese la COLUMNA (1-{total_cols}, -1 para cancelar) para la fila {row}: ", int,
      validator=lambda column: ("Columna fuera de rango.", None) if(column < 1 and column != -1) or column > total_cols else (None, column-1))
    if column == -1:
        return -1, -1
