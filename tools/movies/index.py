"""
Movie utilities for ID generation and management.
Contains functions for creating and managing movie data structure.
"""
...
def create_empty_movies_database() -> MoviesDatabase:
    """
    Creates an empty movies database.
    
    Returns:
        Empty movies database
    """
    return {}
...
def generate_movie_id(title: str, movie_number: int) -> str:
    """
    Generates a unique movie ID based on title and number.
    
    Args:
        title: Movie title
        movie_number: Sequential number for the movie
        
    Returns:
        Movie ID in format: mov_XXX_normalized_title
    """
    # Normalize title: remove special characters and convert to lowercase
    normalized = re.sub(r'[^a-zA-Z0-9]', '_', title.lower())
    normalized = re.sub(r'_+', '_', normalized).strip('_')
    
    # Ensure we have something if title is empty or only special chars
    if not normalized:
        normalized = "pelicula"
    
    # Format: mov_001_titulo_normalizado
...
def create_movie(
    ...
    """
    Creates a movie dictionary with the specified parameters.
    
    Args:
        title: Movie title
        hall: Cinema hall matrix
        category: Movie category (A, B, C, D)
        classification: Age classification
        schedule: Schedule in dd/mm/yyyy format
...
def add_movie_to_database(
    ...
    Args:
        movies_db: Current movies database
        title: Movie title
        hall: Cinema hall matrix
...
def update_movie_field(movies_db: MoviesDatabase, movie_id: str, field: str, value) -> bool:
    ...
    if field in movies_db[movie_id]:
        movies_db[movie_id][field] = value  # type: ignore
        return True
...
def validate_movie_data(
    ...
    if not _validate_date_format(schedule):
