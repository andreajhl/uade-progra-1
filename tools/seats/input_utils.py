"""
Seat utilities for user input and seat finding.
Contains only the functions that are actually used externally.
"""
...
def input_coords_seat(hall: list[list]) -> tuple[int, int]:
    """
...
def input_free_seat(hall: list[list]) -> tuple[int, int]:
    """
...
def find_first_free_seats(hall: CinemaHall, quantity: int) -> list[tuple[int, int]]:
    """
